...
const String httpRequest = "POST / HTTP/1.1\n"
                           "Host: 10.0.0.214\n"
                           "Connection: close\n\n"
                           "{\n"
...
void setup() 
{
  ...
  pinMode(errorPin, OUTPUT); // error LED
  pinMode(buttonPin, INPUT); // momentary button
...
void loop()
{
  ...
  if ((millis() - lastDebounceTime) > debounceDelay) {
    ...
    if (reading != buttonState) {
      ...
      // only toggle the LED if the new button state is LOW (from pushed down to released)
      if (buttonState == LOW) {
        ledState = !ledState;
      }
    }
...
void sendTestData()
{
  ...
  int retVal = client.connect("10.0.0.214", 3131); // Connect to my server

  // const String myMessage = "POST /data HTTP/1.1\n
  //                           Host: localhost:3131\n
...
void clientDemo()
{
  ...
  int retVal = client.connect(destServer, 3131);
  if (retVal <= 0)
  {
